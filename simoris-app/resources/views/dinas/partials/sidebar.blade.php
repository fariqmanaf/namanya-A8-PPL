<aside class="flex flex-col w-[15vw] shadow-lg fixed top-0 bg-white h-screen rounded-r-3xl gap-5 items-center">
  <div class="menu-container flex flex-col mt-20 text-center gap-5">
    ...
    <a href="/dashboard/riwayat" class="p-1 bg-black text-white {{ ($title === "Riwayat Stok") ? 'bg-slate-200 text-black' : '' }}"><button>Riwayat Distribusi</button></a>
    <a class="p-1 bg-black text-white {{ ($title === "Ubah Password") ? 'bg-slate-200 text-black' : '' }}" href="/dashboard/changepass"><button>Ubah Password</button></a>
