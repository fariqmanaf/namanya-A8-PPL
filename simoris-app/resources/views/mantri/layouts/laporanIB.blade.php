...
<div class="content-container w-[85vw] bg-[#DDF2FD] flex flex-col items-center h-full ml-[20vw]">
  <div class="justify-center items-center flex flex-col mt-20 bg-white rounded-2xl w-[85%] 2xl:text-lg">
    ...
    <table class="mt-5 cursor-pointer rounded-xl w-full">
      ...
      <tbody>
        @foreach($dataPeternak as $peternak)
          <tr class="clickable-row border-b text-sm text-left">
            <td class="px-4 py-4">{{ $loop->iteration }}.</td>
            <td class="px-4 py-4">{{ $peternak->individual['name'] }}</td>
            <td class="px-4 py-4">
              {{ $peternak->individual->alamat['detail'] }},
              {{ $peternak->individual->alamat->kelurahan['kelurahan'] }},
              {{ $peternak->individual->alamat->kecamatan['kecamatan'] }},
              {{ $peternak->individual->alamat->kabupaten['kabupaten'] }}
            </td>
            <td class="px-4 py-4">
              <a href="/home/laporanIB/{{ $peternak->individual['id'] }}" class="open-modal">
